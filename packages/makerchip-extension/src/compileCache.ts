 ...
 *
 * Additionally, a compile announces its SandPiper intermediate/output files (top.m4.pre,
 * top.m4, top.sv, top_gen.sv) in metadata.intermediateFiles for agent discovery. These are
 * not streamed with the results; fetchIntermediateFiles() pulls them on demand from the
 * server's results route (/compile/<id>/results/<name>) into this directory by basename.
 *
...
/**
 * A SandPiper intermediate/output file a compile can produce (e.g. top.m4.pre, top.m4,
 * top.sv, top_gen.sv). Unlike the streamed result files, these are NOT sent with the
 * compile results; they are announced here for discovery and pulled on demand from the
 * server's results route into the compile's cache dir (see fetchIntermediateFiles).
 */
interface IntermediateFileInfo {
  description: string;  // What the file is / which pipeline stage produced it
  url: string;          // Server-relative results path: /compile/<id>/results/<name>
  optional?: boolean;   // May legitimately be absent (e.g. top_gen.sv when generation is inlined)
  cached?: boolean;     // True once fetched into the compile's cache dir (local basename === map key)
}
...
interface CompileMetadata {
  ...
  intermediateFiles?: Record<string, IntermediateFileInfo>; // SandPiper intermediate/output files (top.m4.pre, top.m4, top.sv, top_gen.sv), announced for discovery and on-demand fetch. Not streamed with results.
  panelName?: string;       // Panel (webview) that ran this compile, recorded so an intermediate-file fetch can resolve the same server (getServerUrl) without a caller-supplied panel argument.
 ...
 *                 the corresponding file is pre-marked skipped. `panelName` records the
 *                 panel that ran the compile (used to associate the server URL for fetch
 *                 of intermediate files).
 */
export async function initCompile(id: string, sourceCode?: CompileSource, params?: { sim?: boolean; dot?: boolean; panelName?: string }): Promise<void> {
  ...
  // Announce the SandPiper intermediate/output files this compile can produce. They are not
  // streamed with the results; they can be pulled on demand from the server's results route
  // (fetchIntermediateFiles) into this compile's cache dir. Names derive from the top file's
  // base so a non-"top" entry file (e.g. sin_cordic.tlv -> sin_cordic.sv) is honored.
  const topName = typeof sourceCode === 'string' ? 'top.tlv' : (sourceCode?.top ?? 'top.tlv');
  const topBase = path.basename(topName).replace(/\.tlv$/i, '') || 'top';
  const announce = (name: string, description: string, optional = false): [string, IntermediateFileInfo] =>
    [name, { description, url: `/compile/${id}/results/${name}`, ...(optional ? { optional: true } : {}) }];
  metadata.intermediateFiles = Object.fromEntries([
    announce(`${topBase}.m4.pre`, 'After processing `\\TLV` macro syntax and M5 syntactic sugar; fed to M4.'),
    announce(`${topBase}.m4`, 'After all M5 (incl. M4) preprocessing; fed to SandPiper.'),
    announce(`${topBase}.sv`, 'Translated (System)Verilog from SandPiper.'),
    announce(`${topBase}_gen.sv`, 'Generated (System)Verilog from SandPiper (absent when Verilog generation is inlined).', true),
  ]);
...
/**
 * Fetch a compile's announced intermediate/output files from the server into its cache dir.
 *
 * For each entry in metadata.intermediateFiles, GET `${serverUrl}${info.url}`; on success write
 * the body to `<cacheDir>/<name>` and mark `cached: true`. A non-OK response (typically a 404 for
 * an optional file such as top_gen.sv when generation is inlined) leaves the entry un-cached.
 * The network fetches run outside the update queue; only the resulting `cached` flags are written
 * back under serializeUpdate so concurrent result-completion writes aren't clobbered. Idempotent.
 *
 * @returns The updated intermediateFiles map, or undefined if the compile/id is unknown.
 */
export async function fetchIntermediateFiles(id: string, serverUrl: string): Promise<Record<string, IntermediateFileInfo> | undefined> {
  const metadata = await loadMetadata(id);
  if (!metadata?.intermediateFiles) { return undefined; }
  const base = serverUrl.replace(/\/$/, '');
  const compileDir = path.join(CACHE_DIR, id);
  await fs.mkdir(compileDir, { recursive: true });

  const cached: Record<string, boolean> = {};
  for (const [name, info] of Object.entries(metadata.intermediateFiles)) {
    try {
      const res = await fetch(`${base}${info.url}`);
      if (res.ok) {
        await fs.writeFile(path.join(compileDir, name), await res.text(), 'utf-8');
        cached[name] = true;
      } else {
        cached[name] = false;
      }
    } catch (error) {
      console.error(`[compileCache:fetchIntermediateFiles] Failed to fetch ${name} for ${id}:`, error);
      cached[name] = false;
    }
  }

  return serializeUpdate(async () => {
    const m = await loadMetadata(id);
    if (!m?.intermediateFiles) { return undefined; }
    for (const [name, isCached] of Object.entries(cached)) {
      if (m.intermediateFiles[name]) { m.intermediateFiles[name].cached = isCached; }
    }
    await saveMetadata(id, m);
    return m.intermediateFiles;
  });
}
...
export interface RestoreData {
  ...
  /**
   * The server's SandPiper pass/fail verdict, regenerated from the settled cache: a SandPiper-stage
   * failure was recorded as an 'err' event (metadata.error), so restore can replay the failure
   * without re-deriving the exit-code threshold the server owns.
   */
  sandpiperFailed?: boolean;
  /**
...
export async function getRestoreData(id: string): Promise<RestoreData> {
  ...
  return {
    ...
    // A SandPiper-stage failure was recorded at the compilation level (metadata.error); regenerate
    // the verdict from that settled state rather than storing it separately.
    sandpiperFailed: metadata?.error != null && SANDPIPER_ERROR_TYPES.has(metadata.error.type),
    source,
  };
}
