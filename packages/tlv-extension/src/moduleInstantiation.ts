// Module instantiation command (original tl-verilog extension feature).
//
// Node-only: browses the workspace filesystem to pick a Verilog/SystemVerilog
// file and inserts an instantiation snippet. Not part of the web build.
import * as vscode from 'vscode';
import * as fs from 'fs';
import * as path from 'path';

/** Register the "TL Verilog: Instantiate Module" command. */
export function registerModuleInstantiation(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.tlverilog.instantiateModule', instantiateModuleInteract)
  );
}
